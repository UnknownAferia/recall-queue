...
export function createQueueComponents(
  ...
  const queueIsFull = queue.entries.length >= queue.maximumPlayers;

  const queueIsLocked = queue.status !== "open";

  const queueActions = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      ...
      .setDisabled(
        viewerIsQueued || queueIsFull || queueIsLocked || accessSuspended,
      ),
